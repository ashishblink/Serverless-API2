...
export default function TestPixel() {
  ...
  useEffect(() => {
    // Check if authenticated (only on client side)
...
fetch('${process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : (typeof window !== 'undefined' ? window.location.origin : 'YOUR_DOMAIN')}/api/track', {
  method: 'POST',
